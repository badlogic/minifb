...
bool vesa_init(uint32_t width, uint32_t height, uint32_t *actual_width, uint32_t *actual_height) {
	...
	if (!get_info(&vesa_info)) return false;

	int mode_list[256];
	int number_of_modes = 0;
	unsigned long mode_ptr = ((vesa_info.video_mode_ptr & 0xFFFF0000) >> 12) + (vesa_info.video_mode_ptr & 0xFFFF);
	...
	for (int i = 0; i < number_of_modes; i++) {
		...
		// printf("mode: %i, res: %ix%i bpp: %i, mem: %i, planes: %i, bps: %i\n", mode_list[i], mode_info.width, mode_info.height, mode_info.bits_per_pixel, mode_info.memory_model, mode_info.number_of_planes, mode_info.bytes_per_scanLine);

		found_mode = mode_list[i];
		if (mode_info.width == width && mode_info.height == height) break;
	}
	...
	__dpmi_set_segment_base_address(vesa_frame_buffer_selector, vesa_frame_buffer_mapping.address);
	__dpmi_set_segment_limit(vesa_frame_buffer_selector, vesa_frame_buffer_mapping.size - 1);
